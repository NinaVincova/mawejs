...
describe('menu item functionality', () => {
    it('should execute action when menu item is clicked', async () => {
        ...
        const statsc = await $('button[aria-label="Statistics"]');
        await statsc.waitForExist();
        expect(await statsc.isExisting()).to.be.true;
        //await statsc.click();
        // const dialog = await $('.recharts-responsive-container');
        // const dialog = await $('.recharts-responsive-container');
