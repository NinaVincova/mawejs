...
describe('menu item functionality', () => {
    it('should execute action when menu item is clicked', async () => {
        ...
        const sett = await $('button[aria-label="Edit story info"]'); // Selektor tlačidla
        await sett.waitForExist();
        expect(await sett.isExisting()).to.be.true;
        await sett.click();

        const inputsel = await $(`//span[text()="Title"]`);
        await inputsel.waitForExist();
        expect(await inputsel.isExisting()).to.be.true;
